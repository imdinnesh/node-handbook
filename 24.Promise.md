...
When you order the pizza, you're in the **pending** state, hungry and hopeful. If the pizza arrives hot and cheesy, you've entered the **fulfilled** state. But if the restaurant calls to say they've dropped your pizza on the floor, you're in the **rejected** state.
...
Essentially, the code above will execute the same as if you wrote:
...
> [!WARNING]
> When handling a large number of Promises, especially in batch processing, using `Promise.all()` can strain the system's memory.
...
myPromise
  ...
  .finally(() => console.log('Promise completed')); // Runs regardless of resolution
```
