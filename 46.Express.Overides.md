...
1. **Global prototypes** at `express.request` and `express.response`.
...
> [!IMPORTANT]
> Unless necessary, it is recommended that this be done only at the application level, rather than globally. Also, take care that the prototype being used matches the functionality as closely as possible to the default prototypes.
